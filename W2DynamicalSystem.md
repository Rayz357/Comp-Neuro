...
教程一：Juliana Georgieva 将开发基于发射率的网络的概念，并介绍神经元种群的固定点和稳定性的思想。

教程二：扩展到双种群网络模型（Wilson-Cowan模型）。在这一教程中，你将学习如何使用相平面分析来评论由于兴奋和抑制相互作用而引起的动态，例如振荡和吸引子等。
## Tutorial 1: Neural Rate Models
在这个教程中，我们将模拟和研究生物神经网络的最简单模型之一。我们将把神经元视为一个单一的均匀种群，并使用描述其平均发放率随时间演化的单一一维方程来近似其动态。  
1. 编写1D兴奋性种群的发放率动态方程：

这个方程将描述一个兴奋性神经元种群的发放率如何随着时间变化。可以将其视为一个整体的平均描述，而不是单个神经元的具体动态。
2. 使用频率-电流（F-I）曲线可视化种群的响应：

通过分析如阈值和增益等参数的变化，可以了解神经元种群的行为。F-I曲线是神经科学中常用的工具，用于描述神经元或神经元群体对输入电流的响应。
